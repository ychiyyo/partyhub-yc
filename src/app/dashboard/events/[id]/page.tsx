...
export default async function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  event.guests.forEach((guest: import("@prisma/client").Guest) => {
  ...
  const inviteUrl = `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/invite/${event.slug}`; // Note: Guest-specific links will use unique tokens, but this is a generic share link if needed.
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bento-box shadow-lg bg-white/80 border border-zinc-200/50 p-6 md:p-8">
            <div className="flex justify-between items-center mb-6 border-b border-zinc-200/60 pb-4">
              ...
              {/* Optional search/filter can go here */}
            </div>
